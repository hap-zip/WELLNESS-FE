...
export function BrandHeaderLogo({ width = 96 }: { width?: number }) {
  ...
  return (
    <View
      accessibilityLabel="하음"
      accessible
      style={[styles.headerLogo, { width, height: (width * 2) / 3 }]}
    >
      <Image
        accessibilityIgnoresInvertColors
        resizeMode="contain"
...
export function BrandWordmarkImage({ width = 280 }: { width?: number }) {
  const scheme = useAppColorScheme();
  // aspectRatio만 주면 네이티브에서 이미지 원본 크기(1536×1024)를 그대로 쓰려고 해
  // 지정한 width를 무시하고 화면을 뒤덮는 문제가 있었다 — width/height를 둘 다 명시한다.
  return (
    <Image
