...
export default function ConnectionOverlayChart({ labels, metrics, onPointPress, selectedIndex }: { labels: readonly string[]; metrics: readonly VisualConnectionMetric[]; onPointPress: (index: number) => void; selectedIndex: number | null }) {
  return <Svg accessibilityLabel="선택한 기록을 겹쳐 본 그래프" height={188} viewBox="0 0 320 188" width="100%">
    {[0,1,2,3].map(row=><Line key={row} stroke={colors.divider} strokeDasharray="3 5" x1={PADDING} x2={WIDTH-PADDING} y1={PADDING+row*36} y2={PADDING+row*36}/>) }
    {selectedIndex!==null?<Rect fill={colors.dataSoft} height={PLOT_HEIGHT-2*PADDING} opacity={.7} rx={7} width={Math.max(18,(WIDTH-2*PADDING)/labels.length)} x={xAt(selectedIndex,labels.length)-Math.max(9,(WIDTH-2*PADDING)/labels.length/2)} y={PADDING}/>:null}
    ...
    {labels.map((label,index)=>{const show=labels.length<=14||index===0||index===labels.length-1||index%Math.ceil(labels.length/7)===0;return show?<SvgText fill={colors.textMuted} fontSize="8.5" key={`${label}-${index}`} textAnchor="middle" x={xAt(index,labels.length)} y="180">{label}</SvgText>:null})}
  </Svg>;
}
