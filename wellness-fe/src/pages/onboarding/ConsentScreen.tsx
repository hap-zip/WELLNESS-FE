...
export default function ConsentScreen() {
  ...
  const [consents, setConsents] = useState<ConsentState>({ terms: true, privacy: true, health: true, photo: false });
  ...
  return <SafeAreaView edges={['top', 'bottom']} style={styles.screen}>
    <View style={styles.topBar}><NavigationBackButton accessibilityLabel="서비스 안내로 돌아가기" confirmDiscard={hasDraft} fallbackHref="/(onboarding)/intro"/><View style={styles.topSpacer}/></View>
    <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
      <View style={styles.header}><Text accessibilityRole="header" style={styles.title}>약관에 동의하면{`\n`}시작할 수 있어요</Text><Text style={styles.description}>필수 동의와 선택 동의를 나누고, 수집 항목과 목적을 직접 확인해 주세요.</Text></View>
    ...
    <View style={styles.footer}><Pressable accessibilityRole="button" accessibilityState={{ disabled: !requiredChecked }} disabled={!requiredChecked} onPress={() => router.push('/(onboarding)/health-connect')} style={({ pressed }) => [styles.continueButton, !requiredChecked && styles.disabledButton, pressed && styles.pressed]}><Text style={[styles.continueText, !requiredChecked && styles.disabledText]}>필수 동의하고 계속</Text></Pressable></View>
  </SafeAreaView>;
}
