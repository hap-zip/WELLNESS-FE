...
import { BrandWordmark, OnboardingSceneVisual, type OnboardingScene } from '@/components/ui/auth-flow';
...
const SCENES: readonly { id: OnboardingScene; title: string; description: string; note: string }[] = [
  { id: 'record', title: '몸의 하루를\n짧게 남겨요.', description: '수면, 활동, 피부, 불편한 부위를 그날의 상태 그대로 기록해요.', note: '1분 안에 끝나는 데일리 체크' },
...
export default function OnboardingIntroScreen() {
  ...
  return <SafeAreaView edges={['top', 'bottom']} style={styles.screen}>
    <View style={[styles.shell, { width: pageWidth }]}>
      <View style={styles.topBar}><BrandWordmark/><Pressable accessibilityRole="button" onPress={() => router.push('/(onboarding)/consent')} style={({ pressed }) => [styles.skipButton, pressed && styles.pressed]}><Text style={styles.skipText}>안내 건너뛰기</Text></Pressable></View>
      ...
      <ScrollView bounces={false} decelerationRate="fast" horizontal onMomentumScrollEnd={updatePage} pagingEnabled ref={pager} showsHorizontalScrollIndicator={false}>
        {SCENES.map((scene) => <View key={scene.id} style={[styles.page, { width: pageWidth }]}><OnboardingSceneVisual scene={scene.id}/><View style={styles.copy}><Text accessibilityRole="header" style={styles.title}>{scene.title}</Text><Text style={styles.description}>{scene.description}</Text><View style={styles.note}><View style={styles.noteLine}/><Text style={styles.noteText}>{scene.note}</Text></View></View></View>)}
      </ScrollView>
