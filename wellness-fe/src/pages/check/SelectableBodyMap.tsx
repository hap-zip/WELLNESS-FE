...
export function SelectableBodyMap({ selected, view, onChangeView, onToggle }: { selected: string[]; view: BodyView; onChangeView: (view: BodyView) => void; onToggle: (zone: Zone) => void }) {
  ...
  return <View>
    ...
    <View accessibilityLabel={`${view === 'front' ? '앞면' : '뒷면'} 신체 부위 선택`} style={styles.map}>
      <Svg height={328} viewBox="0 0 290 448" width={212}>
        <Circle cx="145" cy="40" fill={colors.border} r="28" />
        <Path d="M115 77 Q145 64 175 77 L193 126 180 252 110 252 97 126Z" fill={colors.border} />
        <Path d="M98 94 68 116 42 226 64 234 100 140Z" fill={colors.border} />
        <Path d="M192 94 222 116 248 226 226 234 190 140Z" fill={colors.border} />
        <Path d="M112 244 86 286 82 430 116 430 143 264Z" fill={colors.border} />
        <Path d="M178 244 204 286 208 430 174 430 147 264Z" fill={colors.border} />
        <Path d="M145 74V252M106 137h78M112 190h66M145 264 116 430M145 264l29 166" fill="none" stroke={colors.surface} strokeWidth="4" />
      </Svg>
      {zones.map((zone) => { const active = selected.includes(zone.id); return <Pressable accessibilityLabel={`${zone.label}${active ? ', 선택됨' : ''}`} accessibilityRole="checkbox" accessibilityState={{ checked: active }} hitSlop={3} key={zone.id} onPress={() => onToggle(zone)} style={[styles.zone, { left: zone.left * MAP_SCALE, top: zone.top * MAP_SCALE, width: zone.width * MAP_SCALE, height: zone.height * MAP_SCALE }, active && styles.selectedZone]}><Text style={styles.zoneText}>{active ? '선택됨' : ''}</Text></Pressable>; })}
    </View>
...
const styles = StyleSheet.create({
  ...
  map: { position: 'relative', width: 212, height: 328, alignSelf: 'center', alignItems: 'center', justifyContent: 'center', marginTop: spacing.sm, overflow: 'hidden', borderRadius: 20, backgroundColor: colors.surfaceSubtle },
  ...
  selectedZone: { borderWidth: 2, borderColor: colors.body, backgroundColor: 'rgba(255,59,59,.17)' },
  zoneText: { color: colors.body, fontSize: 8, fontWeight:'700' },
