...
export default function ReportSetupScreen() {
  ...
  return <SafeAreaView edges={['top']} style={styles.screen}>
    <PageHeader backLabel="이전 화면으로" fallbackHref="/(tabs)/records" title="요약 설정" />
    <ScrollView contentContainerStyle={[styles.content, { paddingBottom: insets.bottom + 120 }]}>
      <Text style={styles.eyebrow}>기록 요약</Text><Text style={styles.title}>공유할 기록을 선택하세요</Text><Text style={styles.description}>진료나 상담 전에 필요한 기간과 항목만 한 장으로 정리합니다.</Text>
    ...
    <View style={[styles.action, { paddingBottom: Math.max(insets.bottom, 12) }]}><Pressable accessibilityRole="button" accessibilityState={{ disabled: count === 0 || invalidRange }} disabled={count === 0 || invalidRange} onPress={next} style={[styles.primary, (count === 0 || invalidRange) && styles.disabled]}><Text style={styles.primaryText}>요약 미리보기 · {count}개 항목</Text></Pressable></View>
  </SafeAreaView>;
}
