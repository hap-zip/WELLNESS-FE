...
export default function ReportPreviewScreen() {
  ...
  const [busyAction, setBusyAction] = useState<'copy' | 'save' | 'share' | null>(null);
  const reportText = data ? makeReportText(data) : '';
  const runAction = async (kind: 'copy' | 'save' | 'share', task: () => Promise<void>) => { if (busyAction) return; setBusyAction(kind); setStatus(''); try { await task(); } catch { setStatus('작업을 완료하지 못했어요. 잠시 후 다시 시도해 주세요.'); } finally { setBusyAction(null); } };
  const copy = () => runAction('copy', async () => { await Clipboard.setStringAsync(reportText); setStatus('요약 텍스트를 복사했어요.'); });
  const share = () => runAction('share', async () => { if (data) await Share.share({ title: '몸기록 요약', message: reportText }); });
  const saveImage = async () => {
    await runAction('save', async () => { if (!cardRef.current) return; const permission = await MediaLibrary.requestPermissionsAsync(); if (!permission.granted) { Alert.alert('사진 저장 권한이 필요해요', '설정에서 사진 추가 권한을 허용해 주세요.'); return; } const uri = await captureRef(cardRef, { format: 'png', quality: 1, result: 'tmpfile' }); await MediaLibrary.saveToLibraryAsync(uri); setStatus('요약 이미지를 사진에 저장했어요.'); });
  };

  return <SafeAreaView edges={['top']} style={styles.screen}>
    <PageHeader backLabel="요약 설정으로 돌아가기" fallbackHref="/reports/setup" title="요약 미리보기" />
    {isLoading ? <View style={styles.center}><ActivityIndicator color={colors.primary}/><Text style={styles.loading}>요약을 만드는 중</Text></View> : error || !data ? <View style={styles.center}><Text style={styles.errorTitle}>요약을 만들지 못했어요</Text><Pressable accessibilityRole="button" onPress={() => void reload().catch(() => undefined)} style={styles.retry}><Text style={styles.retryText}>다시 시도</Text></Pressable></View> : <>
      ...
      <View style={[styles.action, { paddingBottom: Math.max(insets.bottom, 12) }]}><View style={styles.actionRow}><Action busy={busyAction === 'save'} disabled={Boolean(busyAction)} icon="download" label="이미지 저장" onPress={() => void saveImage()}/><Action busy={busyAction === 'copy'} disabled={Boolean(busyAction)} icon="copy" label="텍스트 복사" onPress={() => void copy()}/><Action busy={busyAction === 'share'} disabled={Boolean(busyAction)} icon="share" label="공유" onPress={() => void share()}/></View></View>
      <Modal animationType="slide" navigationBarTranslucent={false} onRequestClose={() => setExpertMode(false)} presentationStyle="fullScreen" statusBarTranslucent={false} visible={expertMode}><SafeAreaView edges={['top', 'bottom']} style={styles.expertScreen}><View style={styles.expertTop}><Text style={styles.expertTitle}>전문가에게 보여주기</Text><Pressable accessibilityLabel="전체화면 닫기" accessibilityRole="button" onPress={() => setExpertMode(false)} style={styles.closeButton}><AppIcon color={colors.text} name="close" size={24}/></Pressable></View><ScrollView contentContainerStyle={[styles.expertContent, { paddingBottom: Math.max(insets.bottom, 20) + 20 }]}><ReportCard data={data}/></ScrollView></SafeAreaView></Modal>
    </>}
  </SafeAreaView>;
}
...
function Action({ busy, disabled, icon, label, onPress }: { busy?: boolean; disabled?: boolean; icon: 'copy' | 'download' | 'share'; label: string; onPress: () => void }) { return <Pressable accessibilityRole="button" accessibilityState={{ busy, disabled }} disabled={disabled} onPress={onPress} style={({ pressed }) => [styles.actionItem, disabled && styles.disabledAction, pressed && styles.pressed]}>{busy ? <ActivityIndicator color={colors.primary} size="small"/> : <AppIcon color={colors.text} name={icon} size={21}/>}<Text style={styles.actionText}>{label}</Text></Pressable>; }
