...
export default function PatternDetailScreen() {
  ...
  if (isLoading) return <SafeAreaView edges={['top', 'bottom']} style={styles.center}><ActivityIndicator color="#1257E0" /><Text style={styles.centerText}>패턴 근거를 확인하는 중</Text></SafeAreaView>;
  if (error) return <SafeAreaView edges={['top', 'bottom']} style={styles.center}><Text style={styles.errorTitle}>패턴을 불러오지 못했어요</Text><Pressable onPress={() => void reload().catch(() => undefined)} style={styles.retryButton}><Text style={styles.retryText}>다시 시도</Text></Pressable></SafeAreaView>;
  if (!data) return <SafeAreaView edges={['top', 'bottom']} style={styles.center}><Text style={styles.errorTitle}>해당 패턴을 찾을 수 없어요</Text><Pressable onPress={() => router.dismissTo('/(tabs)/discover')} style={styles.retryButton}><Text style={styles.retryText}>발견으로 돌아가기</Text></Pressable></SafeAreaView>;
  ...
  return <SafeAreaView edges={['top']} style={styles.screen}><View style={styles.topBar}><NavigationBackButton accessibilityLabel="발견 화면으로 돌아가기" fallbackHref="/(tabs)/discover" /><Text style={styles.topTitle}>패턴 상세</Text><View style={styles.spacer} /></View><ScrollView contentContainerStyle={[styles.content, { paddingBottom: insets.bottom + 36 }]} showsVerticalScrollIndicator={false}>
    <Text style={styles.metric}>{data.metric}</Text><Text style={styles.title}>{data.title}</Text><Text style={styles.summary}>{data.summary}</Text>
    <View style={styles.chartCard}><Text style={styles.cardLabel}>최근 7일 비교</Text><Text style={styles.comparison}>{data.comparisonLabel}</Text><View style={styles.legend}><Legend color="#1257E0" label="첫 번째 지표" /><Legend color="#EF7442" label="비교 지표" /></View><WellnessLineChart labels={data.labels} secondaryValues={data.secondaryValues} values={data.primaryValues} /></View>
    ...
    <View style={styles.suggestionCard}><Text style={styles.suggestionLabel}>오늘의 제안</Text><Text style={styles.suggestion}>{data.suggestion}</Text><Text style={styles.disclaimer}>기록을 기반으로 한 생활 참고 정보이며 의료 진단이 아니에요.</Text></View>
