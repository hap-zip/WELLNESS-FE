...
export default function PatternDetailScreen() {
  ...
  return <SafeAreaView edges={['top']} style={styles.screen}><View style={styles.topBar}><NavigationBackButton accessibilityLabel="발견 화면으로 돌아가기" fallbackHref="/(tabs)/discover" /><Text style={styles.topTitle}>패턴 상세</Text><View style={styles.spacer} /></View><ScrollView contentContainerStyle={[styles.content, { paddingBottom: insets.bottom + 36 }]} showsVerticalScrollIndicator={false}>
    <Text style={styles.metric}>{data.metric}</Text><Text style={styles.title}>{data.title}</Text><Text style={styles.summary}>{data.summary}</Text><View style={styles.confidenceCard}><View style={styles.confidenceTop}><Text style={styles.confidenceLabel}>신뢰 상태</Text><Text style={styles.confidenceBadge}>{data.confidenceLabel}</Text></View><Text style={styles.confidenceInfo}>이 결과는 생활 기록에서 함께 나타난 경향을 보여줍니다. 질병의 원인이나 진단을 의미하지 않습니다.</Text></View>
