# 몸기록 개발 인계서

회사와 개인 개발 환경의 Codex가 동일한 기준으로 작업을 이어가기 위한 저장소 기준 문서입니다. 마지막 갱신일은 2026-08-10입니다.

## 현재 기준점

- 기준 브랜치: `develop`
- 현재 기준 커밋: `9e77610 Merge pull request #46 from hap-zip/feat/native-ui`
- 완료된 기능 브랜치: `feat/native-ui`
- Expo SDK: `^54.0.36`
- Expo Router: `~6.0.24`
- React Native: `0.81.5`
- Node 기준: `.nvmrc`의 `24.15.0`
- 패키지 설치: `npm ci`

문서보다 Git 이력이 최신이면 Git 상태를 우선하고 이 문서를 함께 갱신합니다.

기능 범위의 최종 기준은 `/Users/dongseop/Downloads/웰니스 앱 최종 기능 및 UI 설계 명세.pdf`입니다. 현재 구현과의 차이는 `FINAL_FUNCTION_AUDIT.md`에 정리되어 있으며, 화면이 존재하더라도 해당 문서의 누락 기능이 남아 있으면 완료로 보지 않습니다.

화면 개발과 수정은 `REDESIGN_V3.md`를 유일한 시각 기준으로 사용합니다. 현재 UI는 전면 교체 미완료 상태입니다. 기능이 동작하더라도 기존 JSX 배치·카드·히어로·채팅 말풍선을 유지하면 완료로 보지 않습니다. API 경계와 접근성 감사는 `UI_UX_AUDIT.md`를 함께 적용합니다.

## 현재 구현된 화면·기반

- 인증
  - 네이티브 스플래시와 세션 복원 상태를 잇는 타이포 중심 브랜드 장면
  - 이메일 로그인, 체험 진입, 비밀번호 재설정 요청 목업
  - 이름·이메일·비밀번호 확인·필수 약관 검증을 포함한 회원가입
  - 가입 사용자의 온보딩 미완료 상태 저장과 재실행 시 온보딩 복귀
- 온보딩
  - 기록·연결·행동 3장 서비스 안내와 reduced motion 대응
  - 이용약관·개인정보·건강정보 필수 동의와 피부 사진 선택 동의의 개별 상세
  - 평소 수면·불편 부위·활동·알림 시간 기준 상태 설정
  - OS별 Apple 건강·Health Connect 안내, 선택 데이터 범위, 연결 없이 시작
- 네이티브 시간 선택: 취침·기상·알림 시간
- Safe Area와 공통 뒤로가기 UX
- 네이티브 하단 탭: 홈, 기록, 커넥션 뷰, 챗, 마이
- 오늘 홈: 상태 요약, 인사이트, 루틴, 수면 차트, 최근 기록
- 데일리 체크 전 과정
  - 자동 건강 기록 확인
  - 컨디션
  - 불편 부위·강도·느낌
  - 수면 만족도·자세·베개
  - 활동량·피부·메모
  - 기록 완료
- 수면 자세별 SVG 일러스트 선택 카드
- SVG 전신 근육 바디맵
  - `react-native-body-highlighter` 기반 근육별 실제 SVG 경로
  - 기록된 근육만 색상 표시·터치 가능
  - 기록 강도에 따라 노랑·주황·빨강 표시
  - 기록 완료 후 홈 재포커스 시 최신 바디맵 조회
  - 활성 근육 상세 바텀시트
- 도메인·서비스·비동기 조회 계층 분리
- 기록 캘린더 (`LOG-01`)
  - 실제 월별 날짜 계산과 이전·다음 달 이동
  - 기록 상태 점과 날짜 선택
  - 미기록 과거·오늘 날짜 선택 시 해당 날짜의 기록 작성 시작
  - 기록이 있는 오늘 날짜 선택 시 기존 값을 불러와 수정 시작
  - 작성·수정 대상 날짜를 데일리 체크 전 단계와 저장 payload에 유지
  - 월간 기록일·평균 수면·불편일 통계
  - 선택 날짜 요약 및 기록 없는 날짜의 CTA
  - 로딩·오류·빈 상태와 홈 기록 데이터 재조회
- 날짜별 기록 상세 (`LOG-02`)
  - 자동 기록, 불편 부위·강도·느낌, 수면, 활동·피부, 메모 표시
  - 캘린더 선택 날짜와 상세 라우트 연결
  - 로딩·오류·기록 없음 상태와 안전한 뒤로가기
- 발견 (`CON-01`, `CON-02`)
  - 기준 날짜 선택 시 수면·컨디션·활동 차트 동시 갱신
  - 직접 SVG 경로·막대 계산 함수 `mkLine`, `mkBars`
  - 30일 미만에도 비교 차트와 날짜별 값은 제공
  - 누적 기록 30일 이상부터 발견 패턴 목록과 근거·비교 차트·생활 제안 상세 제공
  - 30일 미만에는 남은 기록 일수와 진행률을 표시하고 패턴 상세 직접 접근도 차단
- 루틴 (`RTN-01`, `RTN-02`, `RTN-03`)
  - 상태 기반 오늘의 추천, 단계·주의사항 안내
  - 실제 종료 시각 기준 실행 타이머와 일시정지·재개·건너뛰기
  - 실행 중 이탈 확인, 단계별 SVG 동작 안내
  - 완료 결과 저장과 홈 루틴 카드 완료 상태 갱신
- 피드백·지속 신호 (`FDB-01`, `SGN-01`)
  - 루틴 전후 변화, 현재 불편 강도, 메모 저장
  - 악화·고강도 응답 시 반복 근거와 안전 안내로 연결
  - 진단으로 오해되지 않는 생활 참고·의료 상담 안내
- 기록 요약 (`RPT-01`, `RPT-02`)
  - 7·14·30일 기간과 포함 항목 설정
  - 선택 항목만 반영한 요약 카드 생성
  - React Native 네이티브 공유 시트와 기록 탭 진입점
- 웰니스 챗 기능 골격 (`CHAT-01`, V3 UI 교체 필요)
  - 홈 플로팅 버튼과 `/assistant` 대화 화면 연결
  - 수면·불편·루틴·요약·오늘 기록 의도별 응답
  - 응답 기반 관련 화면 바로가기와 추천 질문
  - 입력 중 상태, 오류 재시도, 키보드·접근성 대응
  - 진단 요청은 지속 신호와 의료 상담 안내로 제한
- 나·설정 (`MYP-01`, `SET-01`, `SET-02`, `SET-03`, `EMP-01`)
  - 프로필·기록 통계와 설정 화면 진입 허브
  - 건강 데이터 연결·항목 선택 목업과 동기화 상태
  - 알림 종류·시간 설정 목업과 저장 상태
  - 동의 변경, 보관 정보, 전체 데이터 삭제 확인 흐름
  - 공통 빈 상태·오류·완료 안내 컴포넌트와 예시 화면

## 데이터 흐름

```text
달력의 선택 날짜·작성/수정 모드
  → DailyCheckContext
  → ActivitySkinCheckScreen.submit()
  → wellnessApi.saveDailyCheck({ date, ...payload })
  → MockWellnessApi의 날짜별 Map에 실행 중 메모리 저장
  → HomeScreen 포커스 시 getHomeSummary() 재조회
  → bodyParts를 bodyHighlights의 muscle slug로 변환
  → HomeAvatar의 SVG 경로 색상·터치 상태 갱신
```

주요 파일:

- `src/context/daily-check-context.tsx` — 작성 중인 체크 데이터
- `src/context/auth-context.tsx` — 세션과 온보딩 완료 상태
- `src/services/auth-api.ts` — 로그인·가입·비밀번호 재설정 API 계약과 목업
- `src/services/session-store.ts` — SecureStore 세션 저장
- `src/components/ui/auth-flow.tsx` — 인증 워드마크·설정 진행률·온보딩 SVG 장면
- `src/domain/wellness.ts` — API 교체를 위한 도메인 타입
- `src/services/wellness-api.ts` — 목업 저장소와 바디맵 매핑
- `src/hooks/use-async-data.ts` — 비동기 조회 및 재조회
- `src/pages/home/HomeAvatar.tsx` — SVG 전신 근육 바디맵
- `src/pages/home/HomeScreen.tsx` — 홈 조회와 포커스 갱신
- `src/pages/check/SleepPostureIllustration.tsx` — 수면 자세 벡터

현재 기록 부위 매핑:

| 기록 값 | SVG 근육 |
| --- | --- |
| 목 | `neck` |
| 어깨 | `trapezius`, `deltoids` |
| 허리 | `obliques` |
| 무릎 | `knees` |
| 손목 | `forearm` |
| 기타 | 바디맵 표시 없음 |

## 아직 목업인 부분

- `MockWellnessApi` 데이터는 앱 프로세스를 종료하면 초기화됩니다.
- 인증 요청은 실제 서버와 연결되지 않았습니다.
- HealthKit·Health Connect 실제 권한과 동기화는 구현하지 않았습니다.
- 건강 데이터와 알림은 설정 저장 목업이며 실제 OS 권한·동기화·예약은 구현하지 않았습니다.
- 웰니스 챗의 공식 의약품 정보와 식약처 출처 연동은 구현하지 않았습니다.
- 전문가용 요약 카드의 이미지 저장·텍스트 복사·개인정보 숨기기·전체화면 모드는 구현하지 않았습니다.
- 홈 일부 문구와 날짜는 목업 데이터입니다.
- 허리는 현재 정면 `obliques`로 임시 매핑되어 있습니다. 후면 바디맵 도입 시 `lower-back`으로 교체해야 합니다.
- 바디맵 오른쪽 요약은 첫 번째 활성 근육을 대표 상태로 표시합니다. 복수 부위 UX를 추가로 설계해야 합니다.

## 다음 개발 우선순위

1. 웰니스 챗의 참고 기록·공식 의약품·식약처 출처 기능 구현
2. 전문가용 상태 요약 카드의 전체 기능 구현
3. 앞·뒤 바디맵, 좌우 부위별 강도, 피부 사진 구현
4. 마이페이지 계정·세부 데이터 관리 기능 구현
5. 실제 HTTP API·인증·영속 저장 구현체 연결

세부 순서와 완료 조건은 `FINAL_FUNCTION_AUDIT.md`의 P0~P3 백로그를 그대로 사용합니다.

## 작업 원칙

- HTML/WebView를 그대로 복사하지 않지만 웹·편집 디자인의 레이아웃 원리는 React Native로 번역해 적극 활용합니다.
- 모든 화면에 Safe Area, 접근성 라벨, 최소 44pt 터치 영역을 적용합니다.
- 입력에는 검증과 로딩·실패 상태를 제공합니다.
- UI에서 서비스 구현체를 직접 하드코딩하지 않고 도메인/API 계약을 통과시킵니다.
- API 응답을 목업하더라도 교체 가능한 저장소 구조를 유지합니다.
- 뒤로가기는 history가 없을 때의 fallback과 입력 손실 확인을 제공합니다.
- 진단이나 치료로 오해될 표현을 추가하지 않습니다.
- Expo 코드를 쓰기 전에 SDK 57 버전 문서를 확인하고, 프로젝트 SDK 54 호환 여부를 별도로 검증합니다.

## 검증 명령

```bash
npx tsc --noEmit
git diff --check
npx expo export --platform ios --output-dir /tmp/wellness-ios-dist
npx expo-doctor
```

현재 `feat/native-ui` 병합 전 검증 결과:

- TypeScript 검사 통과
- `git diff --check` 통과
- iOS Expo 번들 생성 통과

## 새 작업 시작 절차

```bash
git switch develop
git pull --ff-only origin develop
npm ci
git switch -c feat/<작업명>
```

Codex에 첫 요청으로 다음 문장을 사용합니다.

```text
AGENTS.md와 WORK_HANDOFF.md를 전부 읽고 현재 Git 상태를 확인해.
구현 현황과 다음 우선순위를 요약한 뒤 <작업명>을 실제 사용 가능한 수준으로 구현하고 검증해.
```

작업 후 문서의 기준 커밋·완료 범위·남은 작업을 반드시 갱신합니다.
