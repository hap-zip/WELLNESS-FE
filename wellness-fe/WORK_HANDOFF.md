...
- 몸기록 AI 챗봇 (`CHAT-01`)
  - 홈 플로팅 버튼과 `/assistant` 대화 화면 연결
  - 수면·불편·루틴·요약·오늘 기록 의도별 응답
  - 응답 기반 관련 화면 바로가기와 추천 질문
  - 입력 중 상태, 오류 재시도, 키보드·접근성 대응
  - 진단 요청은 지속 신호와 의료 상담 안내로 제한
- 나·설정 (`MYP-01`, `SET-01`, `SET-02`, `SET-03`, `EMP-01`)
  - 프로필·기록 통계와 설정 화면 진입 허브
  - 건강 데이터 연결·항목 선택 목업과 동기화 상태
  - 알림 종류·시간 설정 목업과 저장 상태
  - 동의 변경, 보관 정보, 전체 데이터 삭제 확인 흐름
  - 공통 빈 상태·오류·완료 안내 컴포넌트와 예시 화면
